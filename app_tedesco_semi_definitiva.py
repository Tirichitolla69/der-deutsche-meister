    ...
    "A1": (
        {
            ...
            "sections": (
                ("Mappa", "In una frase principale il primo campo (**Vorfeld**) contiene un solo blocco di informazione; segue il verbo finito, poi il resto: `Heute | lerne | ich Deutsch.` Il blocco iniziale può essere soggetto, tempo, luogo o oggetto. Non è una regola della *seconda parola*: `Am Wochenende` è un unico blocco."),
                ...
                ("Contrasto utile", "La domanda sì/no non ha un Vorfeld: il verbo apre la frase, `Arbeitest du heute?` Le domande con parola interrogativa invece mantengono il modello: `Wann arbeitest du heute?` Non dire *Heute ich arbeite*: il soggetto non può rimanere davanti al verbo quando il tempo occupa il Vorfeld."),
                ("Allenamento", "Trasforma `Ich fahre um acht Uhr zur Arbeit.` mettendo prima `Um acht Uhr`, poi `Zur Arbeit`. Infine rispondi in una frase completa a `Wo wohnst du?`. Prima di parlare, indica a voce: Vorfeld – verbo – resto."),
                ("Autocontrollo", "In `Morgen besucht meine Schwester ihre Freundin` il verbo è `besucht`; `Morgen` è tutto il primo campo. Se riesci a individuare questi due elementi in ogni frase, stai applicando la regola."),
            ),
        },
        {
            "title": "Identità, persone e verbi ad alta frequenza",
            "outcome": "Presentarsi, parlare di persone e possesso usando con sicurezza pronomi, sein, haben e verbi regolari.",
            "sections": (
                ("Mappa", "Impara il nome insieme al pronome e alla forma verbale: `ich bin`, `du bist`, `er/sie/es ist`, `wir sind`, `ihr seid`, `sie/Sie sind`; `ich habe`, `du hast` ecc. La maiuscola in `Sie` segnala la forma di cortesia; il verbo è plurale: `Kommen Sie aus Italien?`"),
                ("Forma e uso", "Per i verbi regolari togli `-en` e aggiungi `-e, -st, -t, -en, -t, -en`: `ich arbeite, du arbeitest, er arbeitet ...` Dopo radici in `-t/-d` compare spesso una `-e-` di appoggio: `du arbeitest`. L'età si esprime con `sein`: `Ich bin 30 Jahre alt`, non con *haben*."),
                ("Blocchi pronti", "Memorizza frasi che puoi adattare: `Ich heiße … und komme aus …`; `Ich wohne in …`; `Ich bin von Beruf …`; `Ich habe zwei Geschwister.` In una conversazione, chiedi una sola informazione per volta e reagisci: `Ah, interessant!` / `Ich auch.`"),
                ("Allenamento", "Scrivi una carta d'identità di cinque frasi e poi cambiala dal punto di vista di una terza persona: `Das ist Elena. Sie ist …` Alterna `er`, `sie` ed `es`, perché *sie* può voler dire lei o loro: il verbo risolve l'ambiguità (`sie ist` / `sie sind`)."),
        ...
        },
        {
            ...
            "outcome": "Riconoscere soggetto e oggetto diretto e scegliere correttamente gli articoli più frequenti.",
            "sections": (
                ("Mappa", "Ogni nome entra nel quaderno con tre dati: `der Tisch – die Tische – il tavolo`; `die Lampe – die Lampen`; `das Buch – die Bücher`. Il genere è lessicale: non indovinarlo dalla traduzione. Tutti i nomi comuni iniziano con la maiuscola."),
                ("Schema", "Al nominativo: `der/die/das/die`. All'accusativo cambia in modo evidente il maschile: `den/die/das/die`; con l'indeterminativo `einen/eine/ein/–`. Quindi: `Der Mann kauft einen Kaffee.` Il soggetto è `der Mann`; ciò che compra è `einen Kaffee`."),
                ("Quando serve", "Cerca l'accusativo dopo verbi come `haben, brauchen, sehen, kaufen, finden, mögen` e dopo preposizioni come `für, ohne, durch, gegen, um`. Non affidarti soltanto all'ordine: `Den Kaffee kauft der Mann` mantiene lo stesso significato perché `den` segnala l'oggetto."),
                ("Allenamento", "Etichetta in `Ich suche den Bahnhof` chi agisce e che cosa è cercato. Poi crea quattro frasi, una per ogni genere, con `Ich habe …` e sostituisci l'articolo definito con quello indeterminativo quando è possibile."),
                ("Autocontrollo", "Dopo `Ich sehe …`, scegli `den neuen Film`, `die neue Stadt`, `das neue Museum`, `die neuen Häuser`. Nota: il cambiamento più visibile è nel maschile; non aggiungere `-n` al nome singolare."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Le W-Fragen richiedono un'informazione: `Wer? Was? Wo? Wohin? Woher? Wann? Wie? Warum?` La formula è `W-Wort + verbo + soggetto + resto`: `Wo wohnst du?` Le domande sì/no iniziano invece con il verbo: `Wohnst du in Köln?`"),
                ...
                ("Risposte naturali", "Una risposta completa riprende il verbo: `Kommst du morgen? – Ja, ich komme morgen.` Per correggere una supposizione, contrasta il dettaglio: `Nein, nicht morgen, sondern am Freitag.` Le particelle `ja, nein, doch` sono importanti: `doch` contraddice una frase negativa, `Du sprichst nicht Deutsch. – Doch!`"),
                ("Allenamento", "Trasforma `Du hast Zeit` in domanda sì/no, poi crea tre W-Fragen con informazioni diverse. Negare: `Ich habe Zeit` → `Ich habe keine Zeit`; `Ich lerne heute` → `Ich lerne heute nicht.` Spiega a voce perché la parola negativa cambia."),
                ("Autocontrollo", "La frase *Ich habe nicht Geld* è errata: per il nome non determinato serve `kein Geld`. In `Ich kaufe keinen Computer` l'accusativo maschile è segnalato da `keinen`."),
            ),
        },
        {
            "title": "La parentesi verbale: modali e prefissi separabili",
            "outcome": "Esprimere capacità, obbligo e intenzione e comprendere frasi in cui un elemento verbale va alla fine.",
            "sections": (
                ("Mappa", "Con `können, müssen, wollen, dürfen, sollen, mögen/möchten` il modale è coniugato in seconda posizione e l'infinito resta alla fine: `Ich muss heute arbeiten.` Questa è la prima forma della **Satzklammer**, la parentesi verbale."),
                ("Prefissi", "Nei verbi separabili il prefisso va alla fine nella principale: `aufstehen → Ich stehe um sieben auf`; `anrufen → Sie ruft ihre Mutter an`. Nel dizionario il separabile è spesso indicato con un accento: `AUFstehen`. Non separare verbi non separabili come `besuchen`: `Ich besuche …`, non *suche … be*."),
                ("Scelte di registro", "`möchte` è la richiesta o desiderio cortese: `Ich möchte einen Termin.` Non è identico a `wollen`, che esprime volontà più diretta: `Ich will gehen.` Per un cartello o un regolamento, `müssen` comunica obbligo; `dürfen` permesso o divieto (`Hier darf man nicht rauchen`)."),
                ("Allenamento", "Completa: `Am Samstag ___ wir im Supermarkt ___. (einkaufen)`; `Du ___ deinen Ausweis ___. (mitbringen / müssen)`. Evidenzia il verbo finito con un colore e il secondo elemento verbale con un altro."),
                ("Autocontrollo", "In `Kannst du mir helfen?` il soggetto è dopo il verbo perché è una domanda; `helfen` è in fondo. Se il secondo pezzo non è alla fine, ricontrolla la frase."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Per il tempo usa blocchi memorizzabili: `am Montag`, `um 8 Uhr`, `von 9 bis 17 Uhr`, `im Mai`, `heute Morgen`, `jeden Tag`. Metti normalmente il tempo nel Vorfeld o prima dei complementi di luogo: `Am Montag arbeite ich im Büro.`"),
                ("Orari e numeri", "I numeri da 21 in poi invertono unità e decine: `einundzwanzig`, `fünfundvierzig`. `halb acht` significa le 7:30, cioè la metà **prima** delle otto; `Viertel nach sieben` 7:15 e `Viertel vor sieben` 6:45. Per fissare un incontro, usa anche la forma digitale: `um sieben Uhr dreißig`."),
                ("Routine", "I verbi di routine rendono la grammatica automatica: `aufstehen, frühstücken, anfangen, fahren, arbeiten, einkaufen, schlafen`. Collega le frasi con `dann` e `danach`, ricordando V2: `Dann fahre ich zur Arbeit.`"),
                ("Allenamento", "Descrivi una giornata con sei frasi: inizia almeno due frasi con un'indicazione di tempo e usa un verbo separabile. Poi fissa un appuntamento: giorno, ora, luogo, durata e conferma: `Passt dir …? – Ja, das passt.`"),
                ("Autocontrollo", "Scrivi 7:30 in due modi e controlla il significato di `halb neun`. Se pensi alle 8:30, trasferisci la logica italiana: in tedesco è 8:30 solo `halb neun`."),
            ),
        },
        {
            "title": "Possesso, istruzioni e cortesia",
            "outcome": "Parlare di ciò che appartiene a qualcuno e dare istruzioni semplici nel registro adeguato.",
            "sections": (
                ("Mappa", "`mein, dein, sein, ihr, unser, euer, Ihr` non concordano con il possessore ma con la cosa posseduta: `sein Bruder`, `seine Schwester`, `sein Kind`. Al maschile accusativo: `meinen Ausweis`. Osserva `euer → eure/eurem`, dove spesso cade la seconda `e`."),
                ...
                ("Cortesia", "Aggiungi `bitte`, `bitte mal`, `können Sie …?` per una richiesta. Un imperativo nudo può suonare brusco fuori da una situazione familiare. Formula sicura: `Können Sie bitte Ihren Namen sagen?`"),
                ("Allenamento", "Descrivi una borsa con cinque oggetti: `Das ist mein … / Das sind meine …` Poi trasforma istruzioni dirette in cortesi: `Öffnen Sie die Tür.` → `Können Sie bitte die Tür öffnen?`"),
                ("Autocontrollo", "`ihre Adresse` può significare la sua (di lei), la loro o la Sua: il contesto e la maiuscola chiariscono. Verifica sempre il nome posseduto prima di scegliere la desinenza."),
            ),
        },
    ),
    "A2": (
        {
            ...
            "sections": (
                ("Mappa", "Il Perfekt è `haben/sein` coniugato in seconda posizione + `Partizip II` alla fine: `Ich habe gearbeitet.` / `Wir sind gefahren.` Nei dialoghi quotidiani è il principale tempo del passato. La parentesi verbale torna: ausiliare all'inizio della struttura, participio in chiusura."),
                ("Costruzione", "Regolari: `ge- + radice + -t` (`gemacht`); molti irregolari: `ge- + radice + -en` (`gesehen, geschrieben`). I verbi in `-ieren` non prendono `ge-`: `studiert`. I prefissi inseparabili `be-, emp-, ent-, er-, ge-, miss-, ver-, zer-` non lo prendono: `besucht, verstanden`. Impara ogni verbo forte in tre forme: `fahren – fuhr – ist gefahren`."),
                ...
                ("Allenamento", "Racconta ieri in quattro sequenze: inizio, spostamento, attività, fine. Evidenzia l'ausiliare e cerchia il participio. Trasforma `Wir fahren nach Hamburg` in Perfekt e spiega perché scegli `sein`."),
                ("Autocontrollo", "`Ich habe nach Hause gegangen` è errato: `Ich bin nach Hause gegangen`. Però `Ich habe den ganzen Tag gearbeitet` è corretto anche se sei andato al lavoro: conta il verbo, non il contesto."),
            ),
        },
        {
            "title": "Dativo, accusativo e destinatario dell'azione",
            "outcome": "Dire chi riceve qualcosa e che cosa viene trasferito, comprendendo articoli e pronomi più comuni.",
            "sections": (
                ...
                ("Verbi da imparare in coppia", "Dativo + accusativo: `geben, schicken, zeigen, erklären, bringen, kaufen`. Solo dativo: `helfen, danken, gefallen, gehören, antworten`. Non dire *ich helfe dich*: `Ich helfe dir`. Impara il verbo con una frase-tipo, non con la sola traduzione."),
                ...
                ("Allenamento", "Completa una mini-consegna: chi invia che cosa a chi? `Der Chef schickt …` Cambia poi i nomi in pronomi: `Der Chef schickt die E-Mail der Kollegin` → `Er schickt sie ihr.`"),
                ("Autocontrollo", "Chiediti sempre due cose: *a chi?* = dativo; *che cosa?* = accusativo. Se il verbo è `helfen`, non serve un secondo oggetto: `Kannst du mir helfen?`"),
            ),
        },
        {
            "title": "Luogo e movimento: le Wechselpräpositionen",
            "outcome": "Distinguere posizione e direzione per descrivere abitazione, città, spostamenti e indicazioni.",
            "sections": (
                ("Mappa", "`an, auf, hinter, in, neben, über, unter, vor, zwischen` possono reggere dativo o accusativo. La domanda decide: **Wo?** posizione → dativo, `Das Bild hängt an der Wand`; **Wohin?** destinazione/cambiamento di posto → accusativo, `Ich hänge das Bild an die Wand`."),
                ("Non è il movimento", "Non chiederti semplicemente se qualcuno si muove. `Ich laufe im Park` risponde *dove?* quindi dativo, anche se cammino. `Ich laufe in den Park` risponde *verso dove?* quindi accusativo. Questa coppia è la prova migliore."),
                ("Preposizioni fisse", "Altre preposizioni richiedono sempre una forma: dativo `aus, bei, mit, nach, seit, von, zu`; accusativo `durch, für, gegen, ohne, um`. Le destinazioni-città senza articolo usano `nach Berlin`; edifici/persone spesso `zu`: `zum Arzt`, `zur Arbeit`."),
                ("Allenamento", "Disegna una stanza semplice e descrivi tre oggetti con `wo?`; poi spostane due con `wohin?`. Produci coppie: `Das Handy liegt auf dem Tisch. – Ich lege das Handy auf den Tisch.`"),
                ("Autocontrollo", "`in dem` diventa spesso `im`; `in das` diventa `ins`; `an dem` → `am`, `an das` → `ans`. La contrazione non cambia il caso: `im` è dativo, `ins` accusativo."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Comparativo: aggettivo + `-er` + `als`: `Berlin ist größer als Bonn.` Uguaglianza: `so … wie`: `Der Kurs ist so interessant wie …` Superlativo attributivo: `der beste Kurs`; avverbiale: `am besten`. Alcuni sono irregolari: `gut – besser – am besten`, `viel – mehr – am meisten`."),
                ("Tre modi per la causa", "`weil` apre una subordinata e manda il verbo alla fine: `Ich lerne Deutsch, weil ich in Deutschland arbeite.` `denn` coordina e conserva V2: `…, denn ich arbeite …` `deshalb` occupa il Vorfeld della nuova principale: `Ich arbeite in Deutschland. Deshalb lerne ich Deutsch.`"),
                ...
                ("Allenamento", "Confronta due città o due lavori con tre frasi: una con `als`, una con `so … wie`, una con superlativo. Poi dai la stessa ragione con `weil`, `denn` e `deshalb` e osserva dove va il verbo."),
                ("Autocontrollo", "*weil ich lerne Deutsch* è errato: `weil ich Deutsch lerne`. *Deshalb ich lerne Deutsch* è errato: `Deshalb lerne ich Deutsch`, perché anche qui vale V2."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Una congiunzione subordinante apre una frase dipendente e sposta il verbo finito alla fine: `Ich weiß, dass der Kurs heute beginnt.` Nel blocco principale il verbo resta al suo posto. Segna sempre la virgola: è parte della grammatica tedesca scritta."),
                ("Funzioni", "`dass` introduce contenuto/opinione: `Ich finde, dass …`; `weil` causa; `wenn` condizione o evento ripetuto/futuro: `Wenn ich Zeit habe, …`; `obwohl` contrasto: `Obwohl es regnet, gehe ich spazieren.` Per un fatto singolo nel passato usa spesso `als`: `Als ich klein war, …`"),
                ("Doppia parentesi", "Con un modale o Perfekt, gli elementi verbali si accumulano alla fine: `…, weil ich morgen arbeiten muss`; `…, weil ich lange gearbeitet habe`. Non aggiungere una coniugazione all'inizio della subordinata."),
                ...
                ("Autocontrollo", "Trova il verbo finito in `Ich bleibe zu Hause, weil ich krank bin.` È `bin`, e chiude la subordinata. In una subordinata breve è facile; allenati anche con quelle lunghe."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "In un servizio pubblico o sanitario chiarisci sempre: chi sei, perché contatti, qual è il problema, che cosa chiedi e quali dati offri. Formula sicura: `Guten Tag, mein Name ist … Ich möchte einen Termin vereinbaren, weil …`"),
                ("Lessico in azione", "Impara le combinazioni: `einen Termin vereinbaren/verschieben/absagen`, `ein Formular ausfüllen`, `einen Antrag stellen`, `eine Krankmeldung abgeben`, `ein Rezept bekommen`. Il verbo dà la struttura; il nome isolato non basta per usare la parola."),
                ("Passato frequente", "Nel parlato `sein, haben` e i modali compaiono anche al Präteritum: `Ich hatte`, `ich war`, `ich konnte`, `ich musste`. Sono più rapidi e naturali di *ich habe gehabt* nei dialoghi. Il Perfekt resta invece normale per molti altri verbi."),
                ("Allenamento", "Scrivi un messaggio di 60–80 parole per spostare un appuntamento: saluto, riferimento, motivazione, nuova proposta, ringraziamento e formula di chiusura. Poi esercita a voce una telefonata con una domanda di chiarimento: `Könnten Sie das bitte wiederholen?`"),
                ("Autocontrollo", "Una richiesta non è un ordine: preferisci `Könnten Sie …?`, `Ich hätte gern …`, `Wäre es möglich, …?` Se manca una delle cinque informazioni della mappa, il destinatario deve fare una domanda in più."),
            ),
        },
        {
            "title": "Pronomi e verbi riflessivi",
            "outcome": "Parlare di sé, routine e relazioni usando i pronomi nel caso giusto e riconoscendo i verbi riflessivi.",
            "sections": (
                ("Mappa", "Accusativo: `mich, dich, ihn, sie, es, uns, euch, sie/Sie`; dativo: `mir, dir, ihm, ihr, ihm, uns, euch, ihnen/Ihnen`. Sostituisci un nome già noto per evitare ripetizioni: `Ich sehe den Arzt. Ich sehe ihn.`"),
                ("Riflessivo", "Alcuni verbi richiedono il pronome riflessivo: `sich freuen, sich interessieren, sich erinnern, sich treffen`. Spesso l'accusativo: `Ich freue mich`; talvolta dativo con un altro oggetto: `Ich wasche mir die Hände` (le mani sono l'accusativo, la persona il dativo)."),
                ("Posizione", "Nella principale il pronome breve va presto dopo il verbo: `Ich treffe mich heute mit Anna.` Con modale: `Ich möchte mich heute mit Anna treffen.` Impara `sich` insieme alla preposizione quando necessaria: `sich freuen auf/über`, `sich interessieren für`."),
                ("Allenamento", "Trasforma cinque nomi in pronomi: `Ich rufe meine Mutter an` → `Ich rufe sie an`. Poi descrivi la mattina con tre riflessivi e crea una domanda: `Worauf freust du dich?`"),
                ("Autocontrollo", "Non ogni azione su di sé è riflessiva come in italiano: si dice `Ich wasche meine Haare` oppure `Ich wasche mir die Haare`, non necessariamente *ich wasche mich die Haare*. Controlla il verbo nel dizionario."),
            ),
        },
    ),
    "B1": (
        {
            ...
            "sections": (
                ("Mappa", "A B1 non basta una successione di frasi corrette: il lettore deve vedere il rapporto logico. Alterna congiunzioni (`weil, obwohl, falls, während`) e avverbi connettivi (`deshalb, trotzdem, außerdem, dann`). Le prime spostano il verbo alla fine; i secondi occupano il Vorfeld e mantengono V2."),
                ("Scopo", "Con lo stesso soggetto usa `um … zu + infinito`: `Ich lerne, um die Prüfung zu bestehen.` Con soggetti diversi usa `damit`: `Ich erkläre es langsam, damit alle es verstehen.` Dopo `ohne … zu` esprimi assenza di azione; dopo `statt/anstatt … zu`, alternativa."),
                ("Condizione e concessione", "`falls` è una condizione prudente: `Falls Sie Fragen haben, …`; `wenn` è più generale. `obwohl` introduce un ostacolo reale, mentre `trotzdem` apre una nuova principale: `Obwohl es regnet, …` / `Es regnet. Trotzdem …`"),
                ...
                ("Autocontrollo", "Se usi `deshalb`, il verbo viene subito dopo: `Deshalb kann ich …`; se usi `weil`, il verbo chiude: `weil ich … kann`. Il significato è simile, la grammatica no."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Il Vorgangspassiv descrive un processo: `werden + Partizip II`: `Die Rechnung wird geprüft.` Il soggetto grammaticale è ciò che subisce l'azione. Al passato: `wurde geprüft`; al Perfekt: `ist geprüft worden`. Con modale: `Die Rechnung muss geprüft werden.`"),
                ("Agente", "Se serve, indica chi agisce con `von` per persone/istituzioni: `Der Antrag wird von der Behörde bearbeitet`; `durch` per mezzo/cause: `durch ein Programm`. Spesso l'agente non è importante: è proprio il motivo per scegliere il passivo."),
                ("Stato", "`sein + Partizip II` non è un processo ma un risultato: `Die Tür ist geöffnet` = la porta è aperta. `Die Tür wird geöffnet` = qualcuno la sta aprendo. Questa distinzione evita molte frasi ambigue in istruzioni e report."),
                ("Allenamento", "Scrivi cinque passaggi per un processo noto (prenotazione, consegna, ricetta): prima attivo, poi passivo. Inserisci un modale e un indicatore di sequenza: `Zuerst …, anschließend …, zum Schluss …`."),
                ("Autocontrollo", "*Die Rechnung wird geprüft worden* è errato. Presente: `wird geprüft`; Perfekt: `ist geprüft worden`. Ricorda: nel Perfekt passivo l'ausiliare è `sein` e `worden` resta alla fine."),
            ),
        },
        {
            ...
            "sections": (
                ...
                ("Come scegliere", "`Der Mann, der dort steht` (soggetto); `der Mann, den ich kenne` (oggetto); `der Mann, dem ich helfe` (dativo). Fai due domande: qual è l'antecedente? Che funzione ha il pronome nella relativa? Non scegliere il caso dal nome nella frase principale."),
                ("Forme utili", "Con preposizione: `Die Firma, bei der ich arbeite`; `Das Thema, über das wir sprechen`. Per intere frasi o parole come `alles, etwas, nichts`, spesso `was`: `Alles, was du sagst, ist wichtig.` Luoghi: `der Ort, wo …` è comune parlato; nello scritto formale preferisci `an dem/in dem …`."),
                ("Allenamento", "Unisci: `Ich suche eine Wohnung. Die Wohnung liegt zentral.` Poi: `Das ist die Frau. Ich habe der Frau geschrieben.` Spiega a voce perché nella seconda relativa serve il dativo."),
        ...
        },
        {
            ...
            "sections": (
                ("Mappa", "Una risposta B1 convincente segue un filo: **tesi** (`Meiner Meinung nach …`), **ragione** (`Das liegt daran, dass …`), **esempio** (`Zum Beispiel …`), **conseguenza** (`Deshalb …`) e, se utile, **concessione** (`Zwar …, aber …` / `Einerseits …, andererseits …`)."),
                ("Registro", "Distingui fatto, opinione e certezza: `Die Statistik zeigt …` non equivale a `Ich finde …`; `wahrscheinlich, vermutlich, möglicherweise` riducono l'affermazione. Per dissentire: `Da bin ich nicht ganz deiner Meinung, weil …` oppure `Ich verstehe deinen Punkt, aber …`"),
                ("Parole di confronto", "Usa `nicht nur … sondern auch`, `sowohl … als auch`, `weder … noch` con elementi paralleli. Evita di mettere molti connettori senza relazione: ogni connettore deve rendere visibile un preciso passaggio di pensiero."),
                ...
                ("Autocontrollo", "Un'opinione senza ragione non è un'argomentazione. Se togli `weil/denn/da` e non cambia nulla, forse hai espresso solo una preferenza: aggiungi una prova o una conseguenza."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Un `Lebenslauf` è cronologico, sobrio e verificabile; un `Anschreiben` seleziona ciò che è rilevante per l'annuncio: motivo, competenze con esempio, disponibilità. Una e-mail professionale ha oggetto informativo, saluto, richiesta/azione, ringraziamento e `Mit freundlichen Grüßen`."),
                ("Lessico azionabile", "Preferisci verbi concreti: `organisieren, betreuen, prüfen, dokumentieren, koordinieren, verantworten`. Non scrivere *ich bin sehr motiviert* e basta: `In meiner bisherigen Tätigkeit habe ich … koordiniert; deshalb kann ich …` collega prova e risultato."),
                ("Colloquio", "Rispondi con struttura breve: situazione – compito – azione – risultato. Chiedi chiarimenti con `Könnten Sie bitte erläutern, …?` e conferma: `Habe ich Sie richtig verstanden, dass …?` Il registro resta formale finché non viene proposto il `du`."),
                ("Riconoscimento: confine importante", "IHK FOSA è un ente per la valutazione dell'equivalenza di molte qualifiche professionali estere nell'area IHK, non un fornitore di corsi di tedesco. Le esigenze linguistiche dipendono da professione, datore di lavoro, autorizzazioni e misura prevista: il corso prepara a leggere e scrivere documenti, non garantisce requisiti legali."),
                ("Allenamento", "Adatta tre righe di CV a un annuncio: usa una competenza, un'attività, un risultato. Poi scrivi un'e-mail per chiedere informazioni su una candidatura, senza allegare dati personali sensibili in un esercizio."),
            ),
        },
        {
            "title": "Strategia B1 nelle quattro abilità",
            ...
            "sections": (
                ("Mappa", "Il livello B1 richiede autonomia su temi familiari e testi chiari. Non studiare grammatica separata dalle abilità: ogni unità deve produrre un testo, una breve interazione, comprensione globale e comprensione mirata."),
                ...
                ("Scrivere e parlare", "Pianifica in 60 secondi: destinatario, scopo, tre punti, chiusura. Dopo il testo controlla nell'ordine: 1 verbo e ordine, 2 caso/preposizione, 3 genere/plurale, 4 connettori, 5 registro. Nell'orale usa riparazioni: `Ich meine …`, `Anders gesagt …`, `Können Sie die Frage bitte wiederholen?`"),
                ...
                ("Autocontrollo", "Un test pratica il formato, ma l'obiettivo è trasferire la lingua. Tieni un quaderno degli errori con: frase sbagliata, correzione, regola personale e una nuova frase tua."),
            ),
        },
        {
            "title": "Aggettivi, genitivo e gruppi nominali",
            "outcome": "Comprendere e produrre sintagmi nominali più precisi in descrizioni e testi formali.",
            "sections": (
                ("Mappa", "La desinenza dell'aggettivo segnala genere, numero e caso. Strategia: prima individua articolo e caso, poi applica il modello. Dopo articolo determinativo molte informazioni sono già sull'articolo: `der gute Plan / den guten Plan / mit dem guten Plan`; dopo `ein` l'aggettivo completa ciò che l'articolo non mostra: `ein guter Plan`, `ein gutes Angebot`."),
                ("Genitivo", "Indica soprattutto possesso o relazione: `die Adresse des Kunden`, `die Dauer der Schulung`. Maschile/neutro: `des + -(e)s` al nome; femminile/plurale: `der`. Nel parlato spesso `von + dativo`, ma il genitivo è comune nei testi formali e nei titoli."),
    ...
    "B2": (
        {
            ...
            "outcome": "Distinguere tra ciò che affermi e ciò che riferisci, attribuendo correttamente informazioni e posizioni.",
            "sections": (
                ("Mappa", "Il Konjunktiv I segnala discorso indiretto: `Der Bericht erklärt, die Kosten seien gestiegen.` Le forme più riconoscibili sono `sei, habe, werde, könne, müsse`. Non rende un'informazione falsa: marca distanza e fonte, essenziale in notizie, verbali e sintesi."),
                ("Procedura", "1. Nomina la fonte (`laut dem Bericht`, `die Sprecherin sagt …`). 2. Trasforma il verbo quando la forma è riconoscibile. 3. Mantieni tempo e significato con coerenza. Se Konjunktiv I e indicativo coincidono, si usa spesso Konjunktiv II: `Sie sagten, sie hätten …`"),
                ("Non confondere", "`Er sagt, er kommt später` è discorso diretto riportato in stile quotidiano; `Er sagt, er komme später` è marcato e formale. `Er käme später` può essere forma sostitutiva o, in un altro contesto, ipotesi: cerca sempre la fonte e l'intenzione comunicativa."),
                ("Allenamento", "Trasforma cinque informazioni di una breve notizia: `Die Firma: „Wir eröffnen im Mai.“` → `Die Firma teilt mit, sie werde im Mai eröffnen.` Inserisci per ogni frase una fonte e non aggiungere inferenze personali."),
                ("Autocontrollo", "Una sintesi corretta separa `il testo afferma` da `io penso`. Sottolinea i verbi di fonte (`berichten, erklären, betonen, behaupten`) e controlla che il lettore sappia sempre a chi attribuire ogni tesi."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Presente/ipotesi: `würde + infinito` è produttivo: `Ich würde den Vorschlag prüfen.` Forme frequenti da sapere: `hätte, wäre, könnte, müsste, sollte, dürfte, möchte`. Condizione: `Wenn ich mehr Zeit hätte, würde ich …`"),
                ("Passato", "Per un'ipotesi o un rimpianto nel passato: `hätte/wäre + Partizip II`: `Ich hätte früher Bescheid geben sollen.` / `Wir wären gern gekommen.` Con modale: `Ich hätte das erledigen können.`"),
                ("Diplomazia", "Il Konjunktiv II attenua: `Könnten Sie …?`, `Wäre es möglich, …?`, `Ich würde vorschlagen, …` Non usare troppi condizionali se vuoi essere chiaro: dopo una richiesta, indica azione e scadenza in modo diretto ma cortese."),
                ("Allenamento", "Scrivi tre coppie realtà/ipotesi: `Ich habe keine Zeit. – Wenn ich Zeit hätte, …` Poi riformula tre ordini come richieste professionali. Infine esprimi un rimpianto per una riunione mancata."),
                ("Autocontrollo", "`würde` non sostituisce automaticamente ogni forma: per `sein, haben, modali` le forme semplici (`wäre, hätte, könnte`) sono in genere più naturali. Scegli la forma che il lettore riconosce subito."),
            ),
        },
        {
            ...
            "sections": (
                ("Mappa", "Una Stellungnahme B2 ha: questione e tesi, due o tre argomenti sviluppati, controargomento trattato lealmente, valutazione e conclusione. Ogni paragrafo compie una funzione sola. Collegamenti possibili: `zunächst, darüber hinaus, allerdings, demgegenüber, folglich, abschließend`."),
                ("Qualità della prova", "Un esempio illustra; un dato supporta; una fonte attribuisce; nessuno dei tre dimostra tutto da solo. Formula con precisione: `Die Grafik legt nahe …`, `Die Autorin argumentiert …`, `Daraus folgt nicht zwingend, dass …` Evita causalità assolute quando hai solo una correlazione."),
                ("Concessione", "Rafforza la tesi riconoscendo un limite: `Zwar …, jedoch …`; `Auch wenn …, bleibt …`; `Es ist einzuräumen, dass …` La concessione non è un riempitivo: deve rispondere a una vera possibile obiezione."),
                ("Allenamento", "Scegli una domanda con due soluzioni. Crea una tabella mentale: criterio, vantaggio, rischio, prova/esempio. Scrivi 180–220 parole e verifica che ogni conclusione risponda a un argomento già presentato."),
        ...
        {
            "title": "Nominalizzazione, stile e densità informativa",
            "outcome": "Leggere e produrre testo formale senza perdere chiarezza, scegliendo consapevolmente tra frase verbale e nominale.",
            "sections": (
                ("Mappa", "Il tedesco formale usa spesso nomi da verbi/aggettivi: `entscheiden → die Entscheidung`, `prüfen → die Prüfung`, `möglich → die Möglichkeit`. Con preposizioni: `bei der Prüfung`, `nach der Entscheidung`, `zur Verbesserung`. Questo concentra informazioni, ma può appesantire il testo."),
                ("Trasformare", "Verbo: `Die Behörde prüft den Antrag.` Nominale: `Die Prüfung des Antrags durch die Behörde …` La seconda forma funziona in titoli, verbali e documenti; la prima è spesso più leggibile in un'e-mail. Non nominalizzare per sembrare più avanzato: prima assicurati che il responsabile dell'azione e il messaggio restino chiari."),
                ("Attributi", "La densità nasce anche da composti e attributi: `die betriebliche Weiterbildungsmaßnahme`, `die im April eingereichten Unterlagen`. Scomponi dal nucleo a destra: `Weiterbildungsmaßnahme` è il nucleo, `betriebliche` lo qualifica."),
                ("Allenamento", "Riscrivi un breve avviso in due registri: e-mail chiara con verbi, nota formale con due nominalizzazioni. Poi accorcia una frase che contiene più di due nomi astratti: una frase più semplice può essere più professionale."),
        ...
        },
        {
            ...
            "sections": (
                ("Mappa", "Leggi in tre passaggi: 1. scopo, titolo, mittente e struttura; 2. tesi e segnali logici; 3. dati/dettagli richiesti dal compito. Leggere ogni parola dall'inizio è inefficiente e fa perdere il filo dei testi B2."),
                ("Composti", "In un composto, l'ultima parola è il nucleo: `Arbeitszeitgesetz` = legge (`Gesetz`) su tempo (`Zeit`) di lavoro (`Arbeit`). Dividi a destra, poi ricostruisci. Cerca anche elementi di raccordo: `Krankenversicherung`, `Betriebsratssitzung`."),
                ("Grafici", "Distingui valore, confronto e interpretazione: `Der Anteil stieg von … auf …`; `liegt bei …`; `ist im Vergleich zu … höher`; `dies könnte darauf hindeuten, dass …`. Un grafico mostra dati, non cause automatiche. Specifica unità, periodo e gruppo di riferimento."),
                ...
                ("Autocontrollo", "Dopo la lettura verifica: quale informazione è esplicita? Quale è un'inferenza mia? Una sintesi B2 affidabile mantiene questa distinzione anche quando usa parole diverse dall'originale."),
            ),
        },
        {
            ...
            "sections": (
                ...
                ("Architettura", "Testo: apertura che inquadra, paragrafi con frase-guida, conclusione proporzionata. Presentazione: `Zunächst …, anschließend …, zum Schluss …`; chiudi con sintesi e domanda. Le formule fisse aiutano solo se il contenuto dopo di esse è specifico e pertinente."),
                ("Revisione in passaggi", "Fai più riletture: 1 compito e completezza, 2 logica/ordine dei paragrafi, 3 fonti ed esempi, 4 verbi e casi, 5 lessico e registro, 6 ortografia/punteggiatura. Nella produzione orale, registra una prova e ascolta pause, chiarezza e ripetizioni."),
        ...
        },
        {
            ...
            "sections": (
                ("Mappa", "Impara il blocco intero: `warten auf + Akk`, `teilnehmen an + Dat`, `sich interessieren für + Akk`, `abhängen von + Dat`, `bitten um + Akk`. La preposizione non si traduce meccanicamente e decide il caso. Un verbo memorizzato senza preposizione è incompleto."),
                ("Cose e persone", "Per una cosa/tema: `Ich warte darauf.` / `Worauf wartest du?` Per una persona: `Ich warte auf ihn.` / `Auf wen wartest du?` Con vocali iniziali compare `r`: `darauf, worüber, daran`. `wofür` e `dafür` sono una sola parola, non *für was* nello scritto curato."),
                ("Posizione", "La preposizione resta legata al suo complemento; con una subordinata spesso appare `daran, dass …`: `Ich denke daran, dass …` In molti casi è più naturale l'infinito con `daran, … zu`: `Ich denke daran, die Unterlagen zu schicken.`"),
                ("Allenamento", "Crea cartoncini fronte/retro: `sich bewerben – um + Akk – Ich bewerbe mich um …`; `sprechen – mit + Dat – Worüber sprechen wir?` Trasforma cinque ripetizioni di un nome-cosa in `da-` e cinque domande in `wo-`."),
                ("Autocontrollo", "Chiedi prima: il riferimento è una persona o una cosa? Poi: quale preposizione e quale caso richiede il verbo? Questa sequenza risolve gran parte degli errori B2 con complementi verbali."),
            ),
        },
    ),
}
...
LABELS = {
    "it": {
        "open": "Approfondimento da manuale", "outcome": "Obiettivo operativo", "note": "L'approfondimento esteso è redatto in italiano; la teoria essenziale sopra resta disponibile nella lingua dell'interfaccia.",
    },
    "en": {
        "open": "Textbook deepening", "outcome": "Practical outcome", "note": "The concise theory above is available in English. Use its examples to complete the practice below.", "practice": "Write two new examples with the same structure, then say them aloud without reading.",
    },
    "es": {
        "open": "Profundización de manual", "outcome": "Objetivo práctico", "note": "La teoría esencial de arriba está disponible en español. Usa sus ejemplos para completar la práctica siguiente.", "practice": "Escribe dos ejemplos nuevos con la misma estructura y luego dilo en voz alta sin leer.",
    },
    "tr": {
        "open": "Ders kitabı derinleştirmesi", "outcome": "Uygulamalı hedef", "note": "Yukarıdaki temel kuram Türkçe kullanılabilir. Aşağıdaki uygulamayı yapmak için örneklerini kullanın.", "practice": "Aynı yapıyla iki yeni örnek yazın, sonra okumadan sesli söyleyin.",
    },
}
...
def render_textbook_unit(st: Any, level: str, topic_index: int, language: str) -> None:
    ...
    expander_title = f"📘 {labels['open']} · {unit['title']}" if language == "it" else f"📘 {labels['open']}"
    with st.expander(expander_title, expanded=False):
        if language != "it":
            st.caption(labels["note"])
            st.write(labels["practice"])
            return
...
# È più utile di una lunga lista di argomenti: lo studente sa cosa deve saper fare.
MISSIONS = (
    Mission("a1-01", "A1", "Presentarsi con naturalezza", "presentarti, fare domande semplici e reagire a una risposta", "sein, heißen, kommen; verbo in posizione 2", "identità, paesi, lingue, numeri", "Ascolta o leggi tre mini-profili e sottolinea: nome, origine, lingua, professione.", "Registra o pronuncia un autoritratto di 45 secondi: nome, città, lavoro/studio, due gusti e una domanda all'interlocutore.", "Cambia l'ordine: Heute wohne ich in … / In meiner Freizeit …", "Riesci a produrre 6 frasi intere senza leggere?", 35),
    Mission("a1-02", "A1", "Gestire la giornata", "raccontare una routine e fissare un appuntamento", "verbi regolari, separabili e modali", "orari, giorni, routine, appuntamenti", "Leggi un'agenda e ricostruisci a che ora iniziano e finiscono tre attività.", "Invia un messaggio di 4–5 frasi per proporre, accettare o spostare un appuntamento.", "Usa prima il tempo: Am Dienstag kann ich …", "In ogni frase il verbo coniugato è davvero al secondo posto?", 40),
    Mission("a1-03", "A1", "Muoversi e acquistare", "chiedere ciò che vuoi, prezzi e indicazioni", "accusativo; möchten, können; imperativo cortese", "negozi, cibo, trasporti, quantità", "Leggi tre cartelli e scegli il servizio o il binario appropriato.", "Fai due turni di dialogo: in panetteria e alla fermata. Usa bitte, gern, noch einmal.", "Aggiungi una domanda di chiarimento: Wo genau? / Wie viel kostet das?", "Sai distinguere der da den in almeno tre frasi?", 40),
    Mission("a1-04", "A1", "Descrivere persone e luoghi", "descrivere un ambiente e parlare di ciò che possiedi", "articoli possessivi, negazione kein/nicht, dativo con mit", "casa, famiglia, mobili, aggettivi frequenti", "Confronta due annunci di appartamento e trova quattro differenze.", "Descrivi la tua stanza o un appartamento ideale in 6 frasi; includi una frase negativa.", "Inserisci mit + dativo: Ich wohne mit …", "Il testo contiene informazioni, non solo una lista di nomi?", 40),
    Mission("a2-01", "A2", "Raccontare esperienze", "dire cosa è successo, quando e con chi", "Perfekt con haben/sein; participi", "viaggi, weekend, esperienze", "Ordina sei eventi di un weekend usando prima, dann, danach, schließlich.", "Racconta un fine settimana in 80–100 parole e fai una domanda finale a un amico.", "Usa almeno due verbi di movimento con sein.", "I participi sono alla fine della frase?", 45),
    Mission("a2-02", "A2", "Spiegare un problema", "descrivere un guasto o una difficoltà e chiedere aiuto", "dativo; preposizioni locali; weil", "salute, casa, officina, assistenza", "Leggi tre richieste di assistenza e abbina problema e soluzione.", "Scrivi un messaggio a un servizio clienti: problema, quando è iniziato, cosa chiedi.", "Aggiungi una causa con weil e manda il verbo in fondo.", "Il destinatario capisce che cosa deve fare?", 45),
    Mission("a2-03", "A2", "Confrontare e consigliare", "esprimere preferenze e dare un consiglio semplice", "comparativo, superlativo, sollen, lieber", "tempo libero, salute, acquisti", "Scegli tra due offerte e giustifica la scelta con tre criteri.", "Consiglia un amico: Was soll er tun? Usa almeno due confronti.", "Usa zwar … aber in una frase, anche se non è obbligatorio.", "Hai dato una ragione e non solo un'opinione?", 45),
    Mission("a2-04", "A2", "Parlare di progetti", "presentare intenzioni, possibilità e condizioni semplici", "futuro con werden; wenn; connettori", "lavoro, corsi, progetti, tempo", "Individua in un annuncio cinque informazioni: requisito, orario, luogo, scadenza, contatto.", "Presenta un piano per il mese prossimo in 90 secondi o 100 parole.", "Usa wenn per collegare condizione e conseguenza.", "Ogni frase lunga è ancora comprensibile a voce?", 45),
    Mission("b1-01", "B1", "Raccontare con struttura", "narrare un evento, evidenziare il punto importante e reagire", "Präteritum di sein/haben/modali; narrazione", "esperienze, imprevisti, emozioni", "Leggi una notizia breve e separa fatti, conseguenze e opinioni.", "Racconta un imprevisto in 130 parole: situazione, svolta, soluzione, lezione imparata.", "Inserisci almeno tre marcatori temporali variati.", "Un lettore può ricostruire chiaramente la sequenza?", 50),
    Mission("b1-02", "B1", "Lavorare e collaborare", "spiegare una procedura e coordinare attività", "passivo presente; infinitive clauses um … zu", "lavoro, sicurezza, istruzioni, strumenti", "Trasforma quattro istruzioni dirette in una procedura neutra.", "Scrivi una procedura di lavoro sicura in 6 passaggi oppure spiega come si fa una prenotazione.", "Usa prima, danach, anschließend, zum Schluss.", "Hai specificato chi fa cosa e in quale ordine?", 50),
    Mission("b1-03", "B1", "Argomentare con equilibrio", "dire ciò che pensi e riconoscere un punto di vista diverso", "weil, obwohl, deshalb, trotzdem; frasi secondarie", "società, digitale, ambiente, scuola", "Classifica otto affermazioni in fatto, opinione, esempio e conseguenza.", "Scrivi un post di forum di 150 parole: posizione, due ragioni, controargomento, conclusione.", "Collega le idee con almeno quattro connettori diversi.", "Dopo obwohl/weil il verbo finisce davvero alla fine?", 55),
    Mission("b1-04", "B1", "Capire e mediare", "trasmettere in modo semplice un'informazione tedesca a un italofono", "pronomi relativi; verbi con preposizione", "avvisi, regole, salute, viaggi", "Leggi un regolamento tedesco e individua obblighi, divieti e eccezioni.", "Spiega in italiano a un amico le tre informazioni operative più importanti; poi formula una domanda in tedesco per verificare se ha capito.", "Non tradurre parola per parola: raggruppa il significato.", "Hai conservato i dettagli che cambiano davvero l'azione?", 50),
    Mission("b2-01", "B2", "Difendere una posizione", "sostenere una tesi con precisione, esempi e concessioni", "connettori argomentativi; zwar … aber; Konjunktiv II", "lavoro ibrido, mobilità, istruzione", "Leggi due opinioni opposte e annota tesi, argomento, prova, limite.", "Prepara un intervento di 2 minuti: tesi, due argomenti, obiezione, risposta e conclusione.", "Usa almeno una concessione: Einerseits … andererseits …", "La conclusione riprende la tesi senza ripeterla uguale?", 60),
    Mission("b2-02", "B2", "Scrivere in registro formale", "scrivere una richiesta o un reclamo professionale persuasivo", "nominalizzazioni frequenti; Konjunktiv II di cortesia; struttura e-mail", "azienda, amministrazione, servizi, candidature", "Analizza due e-mail: oggetto, scopo, richiesta concreta, chiusura.", "Scrivi 180–220 parole a un ufficio/azienda: contesto, problema o richiesta, proposta, scadenza, chiusura.", "Evita saluti informali e indica una richiesta verificabile.", "Il registro resta formale dall'oggetto al saluto?", 65),
    Mission("b2-03", "B2", "Comprendere l'implicito", "distinguere fatti, atteggiamenti e intenzioni in testi complessi", "Konjunktiv I nel discorso indiretto; particelle e sfumature", "media, scienza, cultura, politiche pubbliche", "Leggi un commento e marca ciò che è fatto, citazione, valutazione e ironia possibile.", "Riporta a voce l'opinione di un autore senza attribuirla a te: Der Autor betont, er sei …", "Confronta due fonti: in che cosa cambiano prospettiva e tono?", "Sai dire 'secondo la fonte' anche quando non sei d'accordo?", 60),
    Mission("b2-04", "B2", "Negoziare una soluzione", "interagire spontaneamente, chiarire e arrivare a un compromesso", "ipotesi con Konjunktiv II; frasi relative avanzate", "alloggio, lavoro, progetto, decisioni comuni", "Leggi un caso con interessi in conflitto e annota priorità/non negoziabili di entrambe le parti.", "Fai una simulazione di 3 minuti: proposta, reazione, domanda chiarificatrice, alternativa, accordo.", "Riformula prima l'idea dell'altro: Wenn ich Sie richtig verstehe, …", "Hai chiesto, reagito e costruito una soluzione; non hai solo parlato?", 65),
)


WRITING_PROMPTS = (
    {
        "level": "B1", "title": "Forum: smartphone a scuola", "words": "150–180",
        ...
        "useful": "Meines Erachtens … / Ein wichtiger Vorteil/Nachteil ist … / Zwar …, aber … / Deshalb schlage ich vor, dass …",
    },
    {
        "level": "B1", "title": "Messaggio al gruppo", "words": "120–150",
        "brief": "Il tuo gruppo sta organizzando un viaggio ma il piano iniziale è troppo costoso. Scrivi al gruppo: spiega il problema, confronta due alternative, esprimi una preferenza e chiedi una decisione.",
        "useful": "Ich habe festgestellt, dass … / Im Vergleich dazu … / Wäre es möglich, …? / Wie seht ihr das?",
    },
    {
        "level": "B2", "title": "E-mail formale: corso annullato", "words": "180–220",
        "brief": "Un corso professionale per il quale hai già pagato è stato annullato con poco preavviso. Scrivi all'organizzazione: indica i fatti, spiega le conseguenze, formula una richiesta precisa e proponi una soluzione accettabile.",
        "useful": "Sehr geehrte Damen und Herren, / hiermit möchte ich mich an Sie wenden, weil … / Für mich ergibt sich daraus … / Ich bitte Sie daher, … / Mit freundlichen Grüßen",
    },
    {
        "level": "B2", "title": "Post argomentativo: città senza auto", "words": "180–220",
        "brief": "Una città vuole limitare drasticamente il traffico privato nel centro. Scrivi un contributo per il forum: valuta vantaggi e svantaggi, rispondi a un'obiezione plausibile e difendi una misura concreta.",
        "useful": "Einerseits …, andererseits … / Es lässt sich nicht bestreiten, dass … / Dennoch überwiegt für mich … / Entscheidend wäre, dass …",
    },
    {
        "level": "B2", "title": "Candidatura mirata", "words": "160–200",
        "brief": "Hai trovato un tirocinio in Germania nel tuo settore. Scrivi un'e-mail di candidatura: motivo, competenze dimostrabili, disponibilità e domanda sul processo successivo.",
        "useful": "mit großem Interesse habe ich … / Besonders qualifiziert mich … / Über die Gelegenheit zu einem Gespräch würde ich mich freuen. / Könnten Sie mir bitte mitteilen, …?",
    },
    {
        "level": "B2", "title": "Sintesi e mediazione", "words": "140–180",
        ...
        "useful": "In pratica significa che … / È obbligatorio … / In caso di … bisogna … / Hast du verstanden, wann …?",
    },
)
...
SPEAKING_PROMPTS = (
    {
        ...
        "moves": "Vorschlag: Wie wäre es mit …? | Reazione: Das passt mir leider nicht. | Compromesso: Dann könnten wir … | Conferma: Also treffen wir uns …",
    },
    {
        ...
        "moves": "Ich verstehe, dass … | Für mich ist es schwierig, weil … | Könnten wir vereinbaren, dass …? | Damit wäre ich einverstanden.",
    },
    {
        "level": "B2", "title": "Lavoro ibrido", "scenario": "Nel team alcuni vogliono più giorni da remoto, altri temono una collaborazione peggiore.",
        "goal": "Difendi una proposta equilibrata, rispondi a un'obiezione e chiudi con un compromesso operativo.",
        "moves": "Aus meiner Sicht spricht dafür, dass … | Ich kann den Einwand nachvollziehen. | Entscheidend wäre jedoch … | Wäre ein Pilotprojekt denkbar?",
    },
    {
        ...
        "moves": "Wenn wir die Fahrtzeit berücksichtigen, … | Was wäre dir wichtiger? | Unter der Bedingung, dass …, wäre ich dafür. | Dann halten wir fest, dass …",
    },
)
...
READING_CASES = (
    {
        ...
        "text": "Wegen mehrerer Unfälle wird der Fahrradraum vom 3. bis 10. Mai umgebaut. Fahrräder dürfen in dieser Zeit nicht im Treppenhaus abgestellt werden. Bewohnerinnen und Bewohner können den provisorischen Stellplatz hinter dem Gebäude benutzen. Wer einen besonders großen Fahrradanhänger hat, soll sich bis Freitag bei der Hausverwaltung melden.",
        "questions": (
            ("Qual è il motivo della misura?", "Mehrere Unfälle", ("Ein neues Gesetz", "Ein Umzug")),
            ("Che cosa è vietato temporaneamente?", "Fahrräder im Treppenhaus abstellen", ("Mit dem Fahrrad fahren", "Den Stellplatz benutzen")),
            ("Chi deve contattare l'amministrazione?", "Menschen mit einem großen Fahrradanhänger", ("Alle Bewohner", "Neue Mieter")),
        ),
    },
    {
        "level": "B2", "title": "Commento: formazione continua",
        "text": "Viele Unternehmen werben damit, lebenslanges Lernen zu fördern. In der Praxis werden Weiterbildungskurse jedoch oft genau dann angeboten, wenn die Arbeitsbelastung besonders hoch ist. Das Problem liegt daher weniger in mangelnder Motivation der Beschäftigten als in der Frage, ob Lernen als private Zusatzleistung oder als Teil der Arbeitszeit verstanden wird. Wer Weiterbildung ernst nimmt, muss nicht nur digitale Plattformen bereitstellen, sondern auch Zeitfenster schaffen, in denen Fehler, Fragen und langsames Verstehen erlaubt sind.",
        "questions": (
            ("Qual è la tesi centrale dell'autore?", "Weiterbildung braucht auch geschützte Arbeitszeit", ("Digitale Plattformen sind überflüssig", "Beschäftigte sind unmotiviert")),
            ...
            ("Quale tono ha la frase finale?", "Fordernd und begründend", ("Ironisch und beleidigend", "Rein beschreibend")),
        ),
    },
)
...
CONNECTORS = {
    "sequenza": ("zuerst", "danach", "anschließend", "schließlich"),
    "causa e conseguenza": ("weil", "da", "deshalb", "daher", "folglich"),
    "contrasto": ("aber", "jedoch", "trotzdem", "obwohl", "während"),
    "argomentazione": ("einerseits", "andererseits", "zwar", "dennoch", "außerdem"),
    "registro formale": ("hiermit", "daher", "bezüglich", "insbesondere", "abschließend"),
}


# Interfaccia internazionale del laboratorio. I contenuti in tedesco restano
# identici; le istruzioni, il feedback e l'obiettivo operativo seguono la
# lingua dell'interfaccia, invece di restare in italiano.
LAB_I18N = {
    "en": {
        "title": "Language mastery lab", "tagline": "Turn German knowledge into understanding, spontaneous interaction and clear writing.",
        "today": "Today", "placement": "Placement", "journey": "Journey", "review": "Review", "practice": "Practice", "mock": "B2 mock", "goal": "Goal",
        "completed": "Completed missions", "due": "Cards due", "saved": "Saved productions", "start": "Starting point", "progress": "Communicative pathway progress",
        "mission": "Today's mission", "can_do": "By the end you can", "grammar": "Grammar focus", "lexis": "Topic vocabulary", "understand": "Understand", "produce": "Produce", "improve": "Improve", "checkpoint": "Checkpoint",
        "input": "Study two or three examples related to this mission. Identify the finite verb, the key vocabulary and one connector.",
        "output": "Speak for one minute or write 6–10 sentences about this topic without copying a model answer.",
        "challenge": "Reuse the grammar focus in a second, more personal version.",
        "check": "Before continuing, check word order, noun capitals and whether your message is understandable without context.",
        "done": "I completed the input, produced my own German and checked the result.", "save_mission": "Save mission", "saved_mission": "Mission saved. Return tomorrow for active review.",
        "placement_intro": "This is not a certificate. Answer without a translator so the app can suggest a useful starting point.", "choose": "Choose an answer", "grade": "Check placement", "placement_result": "Suggested starting point", "points": "points out of",
        "review_intro": "Cover the answer first. Active recall is what makes vocabulary available when you speak.", "answer": "Your answer", "check_review": "Check and schedule review", "correct": "Correct", "again": "Review again tomorrow", "no_due": "No cards are due today.",
        "practice_title": "Production practice", "writing": "Writing", "speaking": "Speaking", "writing_task": "Write 180–220 words for a German forum: take a position on hybrid work, develop two reasons, address one objection and end with a concrete proposal.",
        "speaking_task": "Speak for two minutes: negotiate a solution with a colleague about remote work. Make a proposal, react to an objection, ask a clarifying question and summarise an agreement.",
        "useful": "Useful German: Einerseits …, andererseits … / Ich kann den Einwand nachvollziehen, jedoch … / Wäre es denkbar, dass …?",
        "draft": "Your German draft", "save_draft": "Save practice", "practice_saved": "Practice saved locally. Rework it tomorrow without looking at this version first.",
        "mock_intro": "This learning mock is not an official exam. It trains the reading, listening, writing and speaking abilities expected at B2.", "check_mock": "Check reading section", "score": "Objective section", "official": "Official Goethe B2 practice material",
        "mission_titles": ("Introduce yourself naturally", "Manage your daily schedule", "Move around and buy what you need", "Describe people and places", "Tell about experiences", "Explain a problem", "Compare and advise", "Talk about plans", "Tell a structured story", "Work and collaborate", "Argue with balance", "Understand and mediate", "Defend a position", "Write formally", "Understand implicit meaning", "Negotiate a solution"),
    },
    "es": {
        "title": "Laboratorio de dominio", "tagline": "Convierte el conocimiento del alemán en comprensión, interacción espontánea y escritura clara.",
        "today": "Hoy", "placement": "Diagnóstico", "journey": "Itinerario", "review": "Repaso", "practice": "Práctica", "mock": "Simulación B2", "goal": "Objetivo",
        "completed": "Misiones completadas", "due": "Tarjetas pendientes", "saved": "Producciones guardadas", "start": "Punto de partida", "progress": "Progreso del itinerario comunicativo",
        "mission": "Misión de hoy", "can_do": "Al final podrás", "grammar": "Gramática clave", "lexis": "Vocabulario temático", "understand": "Comprende", "produce": "Produce", "improve": "Mejora", "checkpoint": "Control final",
        "input": "Estudia dos o tres ejemplos relacionados con esta misión. Identifica el verbo conjugado, el vocabulario clave y un conector.",
        "output": "Habla durante un minuto o escribe 6–10 frases sobre este tema sin copiar un modelo.",
        "challenge": "Reutiliza la estructura gramatical en una segunda versión más personal.",
        "check": "Antes de seguir, revisa el orden de palabras, las mayúsculas de los sustantivos y si el mensaje se entiende sin contexto.",
        "done": "He completado la comprensión, he producido alemán propio y he revisado el resultado.", "save_mission": "Guardar misión", "saved_mission": "Misión guardada. Vuelve mañana para el repaso activo.",
        "placement_intro": "No es un certificado. Responde sin traductor para que la aplicación sugiera un punto de partida útil.", "choose": "Elige una respuesta", "grade": "Corregir diagnóstico", "placement_result": "Punto de partida sugerido", "points": "puntos de",
        "review_intro": "Cubre primero la respuesta. La recuperación activa hace que el vocabulario esté disponible al hablar.", "answer": "Tu respuesta", "check_review": "Corregir y programar repaso", "correct": "Correcto", "again": "Repásala mañana", "no_due": "No hay tarjetas pendientes hoy.",
        "practice_title": "Práctica de producción", "writing": "Escritura", "speaking": "Hablar", "writing_task": "Escribe 180–220 palabras para un foro alemán: toma posición sobre el trabajo híbrido, desarrolla dos razones, responde a una objeción y termina con una propuesta concreta.",
        "speaking_task": "Habla dos minutos: negocia una solución con un colega sobre el trabajo a distancia. Haz una propuesta, responde a una objeción, formula una pregunta de aclaración y resume el acuerdo.",
        "useful": "Alemán útil: Einerseits …, andererseits … / Ich kann den Einwand nachvollziehen, jedoch … / Wäre es denkbar, dass …?",
        "draft": "Tu borrador en alemán", "save_draft": "Guardar práctica", "practice_saved": "Práctica guardada localmente. Revísala mañana sin mirar primero esta versión.",
        "mock_intro": "Esta simulación didáctica no es un examen oficial. Entrena comprensión lectora, auditiva, escritura y expresión oral de nivel B2.", "check_mock": "Corregir parte de lectura", "score": "Parte objetiva", "official": "Material oficial de práctica Goethe B2",
        "mission_titles": ("Presentarte con naturalidad", "Gestionar tu rutina", "Desplazarte y comprar lo necesario", "Describir personas y lugares", "Contar experiencias", "Explicar un problema", "Comparar y aconsejar", "Hablar de planes", "Contar una historia estructurada", "Trabajar y colaborar", "Argumentar con equilibrio", "Comprender y mediar", "Defender una posición", "Escribir de manera formal", "Comprender lo implícito", "Negociar una solución"),
    },
    "tr": {
        "title": "Dil yeterlilik laboratuvarı", "tagline": "Almanca bilgisini anlama, spontane etkileşim ve açık yazıya dönüştürün.",
        "today": "Bugün", "placement": "Seviye tespiti", "journey": "Yol haritası", "review": "Tekrar", "practice": "Uygulama", "mock": "B2 denemesi", "goal": "Hedef",
        "completed": "Tamamlanan görevler", "due": "Tekrar kartları", "saved": "Kaydedilen üretimler", "start": "Başlangıç noktası", "progress": "İletişimsel yol haritası ilerlemesi",
        "mission": "Bugünün görevi", "can_do": "Görev sonunda yapabilecekleriniz", "grammar": "Dilbilgisi odağı", "lexis": "Konu kelimeleri", "understand": "Anla", "produce": "Üret", "improve": "Geliştir", "checkpoint": "Son kontrol",
        "input": "Bu görevle ilgili iki veya üç örneği inceleyin. Çekimli fiili, temel kelime dağarcığını ve bir bağlacı belirleyin.",
        "output": "Model cevabı kopyalamadan bu konu hakkında bir dakika konuşun veya 6–10 cümle yazın.",
        "challenge": "Dilbilgisi odağını daha kişisel ikinci bir sürümde yeniden kullanın.",
        "check": "Devam etmeden önce kelime sırasını, isimlerin büyük harfini ve mesajın bağlamsız anlaşılabilirliğini kontrol edin.",
        "done": "Girdiyi tamamladım, kendi Almancamı ürettim ve sonucu kontrol ettim.", "save_mission": "Görevi kaydet", "saved_mission": "Görev kaydedildi. Aktif tekrar için yarın dönün.",
        "placement_intro": "Bu bir sertifika değildir. Uygulamanın yararlı bir başlangıç önermesi için çevirmen kullanmadan cevaplayın.", "choose": "Bir cevap seçin", "grade": "Seviye tespitini değerlendir", "placement_result": "Önerilen başlangıç", "points": "puan /",
        "review_intro": "Önce cevabı kapatın. Aktif hatırlama, kelimeleri konuşurken erişilebilir kılar.", "answer": "Cevabınız", "check_review": "Kontrol et ve tekrarı planla", "correct": "Doğru", "again": "Yarın yeniden çalış", "no_due": "Bugün tekrarlanacak kart yok.",
        "practice_title": "Üretim uygulaması", "writing": "Yazma", "speaking": "Konuşma", "writing_task": "Almanca bir forum için 180–220 kelime yazın: hibrit çalışma hakkında görüş bildirin, iki gerekçe geliştirin, bir itiraza yanıt verin ve somut bir öneriyle bitirin.",
        "speaking_task": "İki dakika konuşun: uzaktan çalışma hakkında bir iş arkadaşınızla çözüm müzakere edin. Öneri sunun, itiraza yanıt verin, açıklayıcı soru sorun ve anlaşmayı özetleyin.",
        "useful": "Yararlı Almanca: Einerseits …, andererseits … / Ich kann den Einwand nachvollziehen, jedoch … / Wäre es denkbar, dass …?",
        "draft": "Almanca taslağınız", "save_draft": "Uygulamayı kaydet", "practice_saved": "Uygulama yerel olarak kaydedildi. Yarın önce bu sürüme bakmadan yeniden çalışın.",
        "mock_intro": "Bu eğitim denemesi resmî sınav değildir. B2 düzeyinde okuma, dinleme, yazma ve konuşma becerilerini çalıştırır.", "check_mock": "Okuma bölümünü kontrol et", "score": "Nesnel bölüm", "official": "Resmî Goethe B2 alıştırma materyali",
        "mission_titles": ("Doğal biçimde kendini tanıtmak", "Günlük planını yönetmek", "Ulaşım ve alışveriş", "İnsanları ve yerleri betimlemek", "Deneyimleri anlatmak", "Bir sorunu açıklamak", "Karşılaştırmak ve tavsiye vermek", "Planlardan söz etmek", "Yapılandırılmış hikâye anlatmak", "Çalışmak ve iş birliği yapmak", "Dengeli biçimde tartışmak", "Anlamak ve aracılık etmek", "Bir görüşü savunmak", "Resmî yazmak", "Örtük anlamı kavramak", "Çözüm müzakere etmek"),
    },
}


def _lab_text(language: str, key: str) -> str:
    return LAB_I18N[language][key]


def _international_question_parts(question: dict[str, Any], language: str) -> tuple[str, list[str], str]:
    if question["type"] == "grammar":
        return question["question"], list(question["options"]), question["answer"]
    item = question["item"]
    if question["type"] == "meaning":
        prompts = {"en": f"What does **{item['de']}** mean?", "es": f"¿Qué significa **{item['de']}**?", "tr": f"**{item['de']}** ne demektir?"}
        return prompts[language], [option[language] for option in question["options"]], question["answer"][language]
    prompts = {"en": f"Complete: the German word for **{item[language]}** is …", "es": f"Completa: la palabra alemana para **{item[language]}** es …", "tr": f"Tamamlayın: **{item[language]}** için Almanca kelime …"}
    return prompts[language], [option["de"] for option in question["options"]], question["answer"]["de"]


def _international_cards(vocab_by_level: dict[str, list[dict[str, str]]], language: str) -> list[dict[str, str]]:
    return [
        {"id": f"{level}-{index}", "level": level, "de": item["de"], "translation": item[language]}
        for level in LEVELS
        for index, item in enumerate(vocab_by_level.get(level, []))
    ]


def _render_international_lab(st: Any, state: dict[str, Any], question_bank: list[dict[str, Any]], vocab_by_level: dict[str, list[dict[str, str]]], language: str) -> None:
    text = LAB_I18N[language]
    st.title(text["title"])
    st.caption(text["tagline"])
    current = _current_mission(state)
    completed = len(state["completed_missions"])
    due = sum(1 for card in state["cards"].values() if card.get("next_review", "9999-12-31") <= date.today().isoformat())
    c1, c2, c3, c4 = st.columns(4)
    c1.metric(text["completed"], f"{completed}/{len(MISSIONS)}")
    c2.metric(text["due"], due)
    c3.metric(text["saved"], len(state["writing"]) + len(state["speaking"]))
    c4.metric(text["start"], state["profile"].get("placement") or "—")
    st.progress(completed / len(MISSIONS), text=text["progress"])
    page = st.segmented_control(text["title"], [text["today"], text["placement"], text["journey"], text["review"], text["practice"], text["mock"]], default=text["today"], required=True, key=f"international_lab_{language}", width="stretch")

    if page == text["today"]:
        mission_index = next(index for index, mission in enumerate(MISSIONS) if mission.id == current.id)
        with st.container(border=True):
            st.subheader(f"{text['mission']}: {current.level} · {text['mission_titles'][mission_index]}")
            st.write(f"**{text['can_do']}:** {text['mission_titles'][mission_index]}")
            st.caption(f"{text['grammar']}: {current.grammar}")
            st.markdown(f"**1. {text['understand']}:** {text['input']}")
            st.markdown(f"**2. {text['produce']}:** {text['output']}")
            st.markdown(f"**3. {text['improve']}:** {text['challenge']}")
            st.markdown(f"**4. {text['checkpoint']}:** {text['check']}")
            with st.form(f"international_mission_{current.id}_{language}"):
                completed_task = st.checkbox(text["done"])
                submitted = st.form_submit_button(text["save_mission"], type="primary", icon=":material/check_circle:")
            if submitted and completed_task:
                ...
                _save_state(state)
                st.success(text["saved_mission"], icon=":material/check_circle:")
        return

    if page == text["placement"]:
        st.subheader(text["placement"])
        st.write(text["placement_intro"])
        ...
        if submitted:
            score = sum(answers[q["id"]] == _international_question_parts(q, language)[2] for q in questions)
            level = "A1" if score <= 4 else "A2" if score <= 8 else "B1" if score <= 12 else "B2"
            state["profile"]["placement"] = level
            _save_state(state)
            st.success(f"{text['placement_result']}: **{level}** ({score}/16 {text['points']})", icon=":material/target:")
        return

    if page == text["journey"]:
        st.subheader(text["journey"])
        done = set(state["completed_missions"])
        for index, mission in enumerate(MISSIONS):
            badge = "✓" if mission.id in done else "○"
            with st.expander(f"{badge} {mission.level} · {text['mission_titles'][index]}"):
                st.caption(f"{text['grammar']}: {mission.grammar}")
                st.write(text["output"])
        return

    if page == text["review"]:
        st.subheader(text["review"])
        st.write(text["review_intro"])
        cards = _international_cards(vocab_by_level, language)
        today = date.today().isoformat()
        known = state["cards"]
        due_cards = [card for card in cards if known.get(card["id"], {}).get("next_review", "0000-00-00") <= today]
        unseen = [card for card in cards if card["id"] not in known]
        session_cards = (due_cards + unseen)[:8]
        if not session_cards:
            st.success(text["no_due"], icon=":material/event_available:")
            return
        with st.form(f"international_review_{language}"):
            answers: dict[str, str] = {}
            for card in session_cards:
                st.markdown(f"**[{card['level']}]** {card['translation']} → ?")
                answers[card["id"]] = st.text_input(text["answer"], key=f"international_review_input_{language}_{card['id']}", label_visibility="collapsed")
            submitted = st.form_submit_button(text["check_review"], type="primary", icon=":material/spellcheck:")
        if submitted:
            score = 0
            for card in session_cards:
                answer = _normalise(answers[card["id"]])
                expected = _normalise(card["de"])
                base_word = _normalise(card["de"].split(" ", 1)[-1])
                correct = answer in {expected, base_word}
                known[card["id"]] = _next_review(known.get(card["id"], {}), correct)
                score += int(correct)
                if correct:
                    st.success(f"{text['correct']}: **{card['de']}**")
                else:
                    st.info(f"{text['again']}: **{card['de']}**")
            _save_state(state)
            st.metric(text["review"], f"{score}/{len(session_cards)}")
        return

    if page == text["practice"]:
        st.subheader(text["practice_title"])
        mode = st.segmented_control(text["practice_title"], [text["writing"], text["speaking"]], default=text["writing"], required=True, key=f"international_practice_{language}")
        task = text["writing_task"] if mode == text["writing"] else text["speaking_task"]
        with st.container(border=True):
            st.write(task)
            st.info(text["useful"], icon=":material/lightbulb:")
        with st.form(f"international_practice_form_{language}"):
            draft = st.text_area(text["draft"], height=260, placeholder="Deutsch …")
            submitted = st.form_submit_button(text["save_draft"], type="primary", icon=":material/save:")
        if submitted and draft.strip():
            state["writing" if mode == text["writing"] else "speaking"].append({"date": date.today().isoformat(), "language": language, "mode": mode, "text": draft})
            _touch_study_day(state)
            _save_state(state)
            st.success(text["practice_saved"], icon=":material/save:")
        return
    ...
    if submitted:
        score = sum(answers[q["id"]] == _international_question_parts(q, language)[2] for q in questions)
        state["exam_history"].append({"date": date.today().isoformat(), "language": language, "reading_score": score, "total": len(questions)})
        _save_state(state)
        st.metric(text["score"], f"{score}/{len(questions)}")
...
def _question_parts(question: dict[str, Any]) -> tuple[str, list[str], str]:
    ...
    prompt = f"Completa: il tedesco per **{item['it']}** è …"
...
def _next_review(card_state: dict[str, Any], correct: bool) -> dict[str, Any]:
    """Piccola versione trasparente dello spaced repetition: 1, 3, 7, 14 … giorni."""
...
def _render_overview(st: Any, state: dict[str, Any]) -> None:
    ...
    with st.container(border=True):
        st.subheader(f"Oggi: {current.level} · {current.title}")
        st.write(f"**Alla fine saprai:** {current.can_do}")
        st.caption(f"Sessione consigliata: {current.minutes} minuti · Grammatica: {current.grammar}")
        ...
        with st.form(f"mission_{current.id}"):
            ...
            checked = st.checkbox("Ho controllato forma, connettori e comprensibilità.")
            note = st.text_area("Una frase che voglio ricordare", placeholder="Es.: Einerseits spart Homeoffice Zeit, andererseits …")
        ...
        if submit:
            if not (understood and produced and checked):
                ...
                _touch_study_day(state)
                _save_state(state)
                st.success("Missione registrata. La prossima volta il percorso ti proporrà il passo successivo.", icon=":material/check_circle:")
    ...
    for group in groups:
        st.write(f"**{group.capitalize()}:** " + " · ".join(CONNECTORS[group]))


def _render_placement(st: Any, state: dict[str, Any], question_bank: list[dict[str, Any]]) -> None:
    st.subheader("Diagnostica iniziale")
    st.write("Non è un esame e non certifica un livello. Serve a scegliere un punto di partenza utile: rispondi senza traduttore, poi usa il risultato come ipotesi da verificare nelle prime missioni.")
...
def _render_path(st: Any, state: dict[str, Any]) -> None:
    st.subheader("Percorso A1 → B2")
    st.caption("Ogni blocco chiude un ciclo completo: capisci, osservi, produci, controlli. Non passare oltre perché hai letto la regola: passa oltre quando sai usarla.")
    done = set(state["completed_missions"])
    for level in LEVELS:
        level_missions = [m for m in MISSIONS if m.level == level]
        number_done = sum(m.id in done for m in level_missions)
        with st.expander(f"{level} · {number_done}/{len(level_missions)} missioni concluse", expanded=level == (state["profile"].get("placement") or "A1")):
            for mission in level_missions:
                status = "Completata" if mission.id in done else "Da svolgere"
                with st.container(border=True):
                    st.markdown(f"**{mission.title}** · :{'green' if mission.id in done else 'orange'}-badge[{status}]")
                    st.write(mission.can_do)
                    st.caption(f"Lessico: {mission.lexis} · circa {mission.minutes} min")
...
def _render_review(st: Any, state: dict[str, Any], vocab_by_level: dict[str, list[dict[str, str]]]) -> None:
    ...
    st.write("Prima prova a recuperare la parola senza guardare. Le carte corrette tornano più tardi; quelle incerte tornano domani. È il recupero, non la rilettura, a rendere il lessico disponibile quando parli.")
    ...
    if not selected:
        st.success("Nessuna carta è dovuta oggi. Torna domani o fai una missione di produzione.", icon=":material/event_available:")
        return
    st.caption(f"Sessione: {len(selected)} carte · dovute: {len(due)} · nuove: {max(0, len(selected) - len(due))}")
    with st.form("review_form"):
        answers: dict[str, str] = {}
        for card in selected:
            st.markdown(f"**[{card['level']}]** Qual è il tedesco per: *{card['it']}*?")
    ...
    if submitted:
        ...
        for card in selected:
            ...
            if correct:
                correct_count += 1
                st.success(f"{card['it']} → **{card['de']}**")
            else:
                st.error(f"{card['it']} → **{card['de']}** · scrivila una volta in una frase personale.")
        _touch_study_day(state)
        _save_state(state)
...
def _writing_feedback(text: str, target: str) -> list[str]:
    ...
    feedback = [f"Lunghezza: **{len(words)} parole** · obiettivo {target}."]
    if len(sentences) < 4:
    ...
    if not re.search(r"\b(ich|meiner|meines|aus meiner sicht|meines erachtens)\b", lower):
        feedback.append("Per una presa di posizione, rendi esplicita la tua voce: Meines Erachtens … / Aus meiner Sicht …")
...
def _render_writing(st: Any, state: dict[str, Any]) -> None:
    prompt = st.selectbox("Scegli un compito", WRITING_PROMPTS, format_func=lambda item: f"{item['level']} · {item['title']}", key="writing_prompt")
    with st.container(border=True):
    ...
    if submitted:
        ...
        for item in feedback:
            st.write("- " + item)
        state["writing"].append({"date": date.today().isoformat(), "title": prompt["title"], "words": len(re.findall(r"[A-Za-zÄÖÜäöüß]+", text)), "checked": self_check, "text": text})
        _touch_study_day(state)
        _save_state(state)
...
def _render_speaking(st: Any, state: dict[str, Any]) -> None:
    prompt = st.selectbox("Scegli una situazione", SPEAKING_PROMPTS, format_func=lambda item: f"{item['level']} · {item['title']}", key="speaking_prompt")
    with st.container(border=True):
        ...
        st.markdown("**Metodo:** prepara 30 secondi, parla per 90–120 secondi, riascoltati se puoi, poi ripeti una seconda volta scegliendo due miglioramenti concreti.")
    with st.form("speaking_reflection"):
        ...
        fluency = st.select_slider("Quanto sei riuscito/a a restare in tedesco?", options=["meno di 30 s", "30–60 s", "1–2 min", "oltre 2 min"], value="30–60 s")
        note = st.text_input("Una frase da riusare", placeholder="Wenn ich Sie richtig verstehe, …")
    ...
    if submitted:
        ...
        if score < 3:
            st.info("Rifai lo stesso scenario: il secondo giro è dove trasformi formule passive in riflessi.", icon=":material/replay:")


def _render_reading(st: Any) -> None:
    case = st.selectbox("Scegli il testo", READING_CASES, format_func=lambda item: f"{item['level']} · {item['title']}", key="reading_case")
...
def _render_production(st: Any, state: dict[str, Any]) -> None:
    mode = st.segmented_control("Scegli l'abilità da allenare", ["Scrittura", "Parlato", "Lettura critica"], default="Scrittura", required=True, key="production_mode", width="stretch")
    if mode == "Scrittura":
...
def _render_b2_mock(st: Any, state: dict[str, Any], question_bank: list[dict[str, Any]]) -> None:
    ...
    st.write("Questa è una simulazione didattica, non un esame ufficiale. Allena le quattro competenze valutate nel Goethe-Zertifikat B2: Lesen, Hören, Schreiben e Sprechen.")
    ...
    with st.expander("2. Hören · allenamento concreto"):
    ...
    with st.expander("3. Schreiben · 35 minuti"):
        ...
        st.markdown("**Autovalutazione B2:** posizione chiara · ragioni sviluppate · esempio pertinente · concessione/controargomento · richiesta o conclusione operativa · errori che non impediscono la comprensione.")
    with st.expander("4. Sprechen · 15 minuti"):
        ...
        st.markdown("**Autovalutazione B2:** parlo senza leggere · collego le idee · reagisco · chiarisco fraintendimenti · sintetizzo un accordo.")
...
def render_fluency_lab(st: Any, question_bank: list[dict[str, Any]], vocab_by_level: dict[str, list[dict[str, str]]], language: str = "it") -> None:
    """Entry point chiamato da app.py; lo stato è per utente e viene anche salvato localmente."""
    state = _load_state(st)
    if language != "it":
        _render_international_lab(st, state, question_bank, vocab_by_level, language)
        return
...
LAB_SECTION_LABEL = {
    "it": "Laboratorio di padronanza · pratica attiva",
    "en": "Language mastery lab · active practice",
    "es": "Laboratorio de dominio · práctica activa",
    "tr": "Dil yeterlilik laboratuvarı · aktif uygulama",
}

sections = {
    "lab": LAB_SECTION_LABEL[language],
...
if section == "lab":
    render_fluency_lab(st, QUESTION_BANK, VOCAB_BY_LEVEL, language)
